...
from dmbw612.models import CNNSequential
...
if __name__ == '__main__':
    ...
    if training:
        cnn = CNNSequential(input_shape=input_shape, num_classes=num_classes)
        """
        We used from_logits=True because model does not use softmax.
        We used CategoricalAccuracy because model receives one-hot encoded labels.
        Use SparseCategoricalAccuracy, if you want to use integer labels like [1, 9, 3, ..., 0]
        """
        cnn.compile(
        ...
        cnn2 = CNNSequential(input_shape=input_shape, num_classes=num_classes)
        cnn2.compile()
        ...
        cnn2.load_weights(filepath=latest_path)
