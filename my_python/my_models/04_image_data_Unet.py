...
from dmbw612.models import ResNet

# 텐서플로우, 케라스 버전체크
print(f'tf.version {tf.__version__}')
print(f'keras.version {keras.__version__}\n')

# 체크포인트 콜백 생성을 위해서, 체크포인트를 저장할 디렉토리를 먼저 생성.
fullpath = Path(__file__)
file = Path(__file__).name
filename = Path(__file__).stem
parent_dir = Path(__file__).parent.parent.resolve()
print(filename, parent_dir)
...
if __name__ == '__main__':
    ...
    if training:
        resnet.save_weights(filepath=ckpt_filepath.__str__().format(epoch=0))
        history = resnet.fit(
            train_ds,
            epochs=3,
        ...
        resnet.load_weights(filepath=latest_path)
